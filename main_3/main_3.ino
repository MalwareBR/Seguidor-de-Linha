...
const int dirRetoE   = 1;  // esquerda frente
const int dirAtrasE  = 23;   // esquerda ré    (TX0)

// Velocidades/limites
float porCurva = 100.0;
float porReta  = 100.0;
float porMax   = 100.0;
...
// Mantissas + expoentes
float Kp = 6.7f; int multiP = -3;
float Ki = 0.0f; int multiI = 1;
float Kd = 2.2f; int multiD = -4;
// Derivativo filtrado
...
volatile uint32_t contFitaEsq = 0; volatile uint32_t contFitaDir = 0;
...
void sendTelemetryPacket() {
  ...
  d["pwmD"] = g_pwmD_pct;
  d["fitaEsq"] = contFitaEsq;
  d["fitaDir"] = contFitaDir;
  JsonArray s = d.createNestedArray("s");
  for (uint8_t i = 0; i < SensorCount; i++) {
    s.add(sensorValues[i]);   // 0 (branco) .. 1000 (preto) — conforme QTR
  }
...
void checkSinais() {
  ...
  if ((sensorValues[14] < SINAL_LIMTE  || //Extremo direito
      sensorValues[15]  < SINAL_LIMTE) && //Extremo direito
      (sensorValues[7]  < SINAL_LIMTE  || //Centro
      sensorValues[8]   < SINAL_LIMTE) && //Centro
      sensorValues[0]   > SINAL_LIMTE  && //Extrema esquerda
      sensorValues[1]   > SINAL_LIMTE  && //Extrema esquerda
      (sensorValues[12] > SINAL_LIMTE  || //Meio da direita
      sensorValues[11]  > SINAL_LIMTE  || //Meio da direita
      sensorValues[10]  > SINAL_LIMTE)) { //Meio da direita
